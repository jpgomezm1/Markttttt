...
def update_wishlist(request,wishlist_id):
    ...
    try:
        ...
        if form.is_valid():
            ...
            return Response({'ERROR': 'No se pudo actualizar la wishlist','status':'error'},status=HTTP_400_BAD_REQUEST)
    except Exception as e:
        return get_error(e)
...
def add_address(request):
    try:
        ...
        if form.is_valid():
            ...
            return Response({'message': 'Formulario no valido','status':'error'},status=HTTP_400_BAD_REQUEST)
    except Exception as e:
        return get_error(e)
